...
class RandomAttacker(BasicAttacker):
    def __init__(self, attacker_config):
        ...
        self.candidates = set(range(self.n_items)) - {self.target_item}
    ...
    def generate_fake_users(self, verbose=True, writer=None):
        ...
        for f_u in range(self.n_fakes):
            filler_items = np.random.choice(self.candidates, size=self.n_inters - 1, replace=False)
            self.fake_users[f_u, filler_items] = 1.
            self.fake_users[f_u, self.target_item] = 1.
...
class BandwagonAttacker(BasicAttacker):
    def __init__(self, attacker_config):
        ...
        self.n_popular_inters = int(self.n_inters * attacker_config['popular_inter_rate'])
        ...
        self.popular_candidates = np.array(list(set(popular_items) - {self.target_item}))
        self.unpopular_candidates = np.array(list(set(unpopular_items) - {self.target_item}))
    ...
    def generate_fake_users(self, verbose=True, writer=None):
        ...
        for f_u in range(self.n_fakes):
            ...
            filler_items = np.random.choice(self.unpopular_candidates,
                                            size=self.n_inters - self.n_popular_inters - 1, replace=False)
