...
def main():
    ...
    ax1.plot(np.arange(len(epochs)), np.array(full_retrain), 's--', markersize=3, label='Original training', color='#b0d0ff', linewidth=3)
    ax1.plot(np.arange(len(epochs)), np.array(full_retrain_wt_gp), 'x-', markersize=3, label='Training enhanced by GP', color='#bcf4b5', linewidth=3)
    ...
    ax1.grid(True, which='both', linestyle='--', linewidth=0.2, alpha=0.5)
    ax1.minorticks_on()
    ...
    epochs = [1, 5, 10, 50, 100]
    ax2.plot(np.arange(len(epochs)), np.array(full_retrain), 's--', markersize=3, label='Original training', color='#b0d0ff', linewidth=3)
    ax2.plot(np.arange(len(epochs)), np.array(full_retrain_wt_gp), 'x-', markersize=3, label='Training enhanced by GP', color='#bcf4b5', linewidth=3)
    for i, txt in enumerate(full_retrain):
    ...
    ax2.grid(True, which='both', linestyle='--', linewidth=0.2, alpha=0.5)
    ax2.minorticks_on()
    ...
    epochs = [1, 5, 10, 50, 100]
    ax3.plot(np.arange(len(epochs)), np.array(full_retrain), 's--', markersize=3, label='Original training', color='#b0d0ff', linewidth=3)
    ax3.plot(np.arange(len(epochs)), np.array(full_retrain_wt_gp), 'x-', markersize=3, label='Training enhanced by GP', color='#bcf4b5', linewidth=3)
    for i, txt in enumerate(full_retrain):
    ...
    ax3.grid(True, which='both', linestyle='--', linewidth=0.2, alpha=0.5)
    ax3.minorticks_on()
    ...
    epochs = [1, 5, 10, 50, 100]
    ax4.plot(np.arange(len(epochs)), np.array(full_retrain), 's--', markersize=3, label='Original training', color='#b0d0ff', linewidth=3)
    ax4.plot(np.arange(len(epochs)), np.array(full_retrain_wt_gp), 'x-', markersize=3, label='Training enhanced by GP', color='#bcf4b5', linewidth=3)
    for i, txt in enumerate(full_retrain):
    ...
    ax4.grid(True, which='both', linestyle='--', linewidth=0.2, alpha=0.5)
    ax4.minorticks_on()
    ...
    ax1.plot(np.arange(len(epochs)), np.array(full_retrain), 's--', markersize=3, label='Original training', color='#B1BCC6', linewidth=3)
    ax1.plot(np.arange(len(epochs)), np.array(full_retrain_wt_gp), 'x-', markersize=3, label='Training enhanced by GP', color='#CBD8C3', linewidth=3)
    for i, txt in enumerate(full_retrain):
        ax1.text(i + 0.1, full_retrain[i] - 0.01, '{:.1f}s'.format(full_retrain_time[i]),
                 fontsize=8, verticalalignment='bottom', horizontalalignment='right')
    for i, txt in enumerate(full_retrain_wt_gp_time):
    ...
    epochs = [1, 5, 10]
    ax2.plot(np.arange(len(epochs)), np.array(full_retrain), 's--', markersize=3, label='Original training', color='#B1BCC6', linewidth=3)
    ax2.plot(np.arange(len(epochs)), np.array(full_retrain_wt_gp), 'x-', markersize=3, label='Training enhanced by GP', color='#CBD8C3', linewidth=3)
    for i, txt in enumerate(full_retrain):
        ax2.text(i + 0.1, full_retrain[i] - 0.01, '{:.1f}s'.format(full_retrain_time[i]),
                 fontsize=8, verticalalignment='bottom', horizontalalignment='right')
    for i, txt in enumerate(full_retrain_wt_gp_time):
    ...
    epochs = [1, 5, 10]
    ax3.plot(np.arange(len(epochs)), np.array(full_retrain), 's--', markersize=3, label='Original training', color='#B1BCC6', linewidth=3)
    ax3.plot(np.arange(len(epochs)), np.array(full_retrain_wt_gp), 'x-', markersize=3, label='Training enhanced by GP', color='#CBD8C3', linewidth=3)
    for i, txt in enumerate(full_retrain):
        ax3.text(i + 0.1, full_retrain[i] - 0.01, '{:.1f}s'.format(full_retrain_time[i]),
                 fontsize=8, verticalalignment='bottom', horizontalalignment='right')
    for i, txt in enumerate(full_retrain_wt_gp_time):
    ...
    epochs = [1, 5, 10]
    ax4.plot(np.arange(len(epochs)), np.array(full_retrain), 's--', markersize=3, label='Original training', color='#B1BCC6', linewidth=3)
    ax4.plot(np.arange(len(epochs)), np.array(full_retrain_wt_gp), 'x-', markersize=3, label='Training enhanced by GP', color='#CBD8C3', linewidth=3)
    for i, txt in enumerate(full_retrain):
        ax4.text(i + 0.1, full_retrain[i] - 0.01, '{:.1f}s'.format(full_retrain_time[i]),
                 fontsize=8, verticalalignment='bottom', horizontalalignment='right')
    for i, txt in enumerate(full_retrain_wt_gp_time):
    ...
    ax1.plot(epochs, grad_sims[:, 0], '-', markersize=0, label='Interacted user-item pairs', color='#b0d0ff', linewidth=3)
    ax1.fill_between(epochs, grad_sims[:, 0] - grad_sims[:, 1], grad_sims[:, 0] + grad_sims[:, 1], alpha=0.5, color='#ddebf7')
    ax1.plot(epochs, grad_sims[:, 2], '-', markersize=0, label='Random user-item pairs', color='#bcf4b5', linewidth=3)
    ax1.fill_between(epochs, grad_sims[:, 2] - grad_sims[:, 3], grad_sims[:, 2] + grad_sims[:, 3], alpha=0.5, color='#e2f0d9')
    ...
    epochs = np.arange(grad_sims.shape[0])
    ax2.plot(epochs, grad_sims[:, 0], '-', markersize=0, label='Interacted user-item pairs', color='#b0d0ff', linewidth=3)
    ax2.fill_between(epochs, grad_sims[:, 0] - grad_sims[:, 1], grad_sims[:, 0] + grad_sims[:, 1], alpha=0.5, color='#ddebf7')
    ax2.plot(epochs, grad_sims[:, 2], '-', markersize=0, label='Random user-item pairs', color='#bcf4b5', linewidth=3)
    ax2.fill_between(epochs, grad_sims[:, 2] - grad_sims[:, 3], grad_sims[:, 2] + grad_sims[:, 3], alpha=0.5, color='#e2f0d9')
    ax2.set_xlabel('Training Epoch', fontsize=18)
    ax2.set_ylim(-0.3, 0.8)
    ax2.tick_params(axis='both', labelsize=18)
    ax2.set_title('Yelp', fontsize=18)
    ax2.grid(True, which='both', linestyle=':', linewidth=0.8, alpha=0.5)
    ax2.minorticks_on()
    ax2.tick_params(which='both', direction='in')
    ax2.xaxis.set_ticks_position('both')
    ax2.yaxis.set_ticks_position('both')
    ...
    ax2.plot(np.arange(len(alpha_value)), alpha_odd_mean, 's-', markersize=3,
             label='$\\alpha_{odd}$', color='#b0d0ff', linewidth=3)
    ax2.plot(np.arange(len(alpha_value)), alpha_even_mean, 'x-', markersize=3,
             label='$\\alpha_{even}$', color='#bcf4b5', linewidth=3)
