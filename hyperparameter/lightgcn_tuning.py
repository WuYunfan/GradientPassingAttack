...
def fitness(lr, l2_reg, n_layers):
    set_seed(2021)
    device = torch.device('cuda')
    dataset_config = {'name': 'ML1MDataset', 'min_inter': 10, 'path': 'data/ML1M',
                      'split_ratio': [0.8, 0.2], 'device': device}
    model_config = {'name': 'LightGCN', 'embedding_size': 64, 'n_layers': n_layers, 'device': device}
...
def main():
    ...
    best_params = None
    for params in grid:
        ndcg = fitness(params['lr'], params['l2_reg'], params['n_layers'])
