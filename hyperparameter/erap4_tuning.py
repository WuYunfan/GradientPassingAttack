...
def fitness(lr, momentum, s_lr, s_l2, propagation_order, weight, n_pretrain_epochs):
    set_seed(2021)
    ...
    surrogate_trainer_config = {'name': 'EPAR4Trainer', 'optimizer': 'Adam', 'lr': 1.e-2, 'weight': weight,
                                'l2_reg': s_l2,  'n_epochs': n_pretrain_epochs, 'test_batch_size': 2048}
    attacker_config = {'name': 'ERAP4', 'lr': lr, 'momentum': momentum, 's_lr': s_lr,
                       'device': device, 'n_fakes': 131, 'unroll_steps': 3, 'train_epochs': 10,
                       'n_inters': 41, 'topk': 20, 'kappa': 1., 'adv_epochs': 30, 'sample_ratio': 1,
...
def main():
    ...
    param_grid = {'lr': [0.1, 1., 10.], 's_lr': [1.e-2, 1.e-3], 's_l2':  [1.e-1, 1.e-2, 1.e-3],
                  'momentum': [0., 0.9, 0.99], 'propagation_order': [2], 'weight': [20.],
    ...
    for params in grid:
        hr = fitness(params['lr'], params['momentum'], params['s_lr'], params['s_l2'],
                     params['propagation_order'], params['weight'], params['n_pretrain_epochs'])
